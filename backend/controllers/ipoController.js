...
export async function createIPO(req, res) {
  try {
    ...
    // Normalize path to URL-style path starting with /uploads/
const logo_path = req.files['logo']?.[0]?.filename
  ? `/uploads/${req.files['logo'][0].filename}`
  : null;

const rhp_path = req.files['rhp']?.[0]?.filename
  ? `/uploads/${req.files['rhp'][0].filename}`
  : null;

const drhp_path = req.files['drhp']?.[0]?.filename
  ? `/uploads/${req.files['drhp'][0].filename}`
  : null;
